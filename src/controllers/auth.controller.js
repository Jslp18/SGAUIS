...
export const registrarUsuario = async (req, res) => {
  ...
  const newUser = new Users({ codigo, nombre, correo, password: await Users.encryptContraseña(password), rol }) // Creando el usuario y encriptando su comtraseña por medio de la función encryptContraseña declarada en el modelo "Users"
  const foundCodigo = await Users.findOne({ codigo: req.body.codigo }) // Consulta para verificar que el código no exista

  if (foundCodigo) {
    res.status(409).json('El código ya está en uso. Por favor, elige otro.') // Si existe mensaje que indica que el código está en uso
  } else {
    if (rol) {
      ...
      if (foundRol) {
        ...
        return res.status(400).json('El rol especificado no existe. Por favor, verifica e intenta de nuevo.') // De lo contrario mensaje que indica que no existe el rol
      }
    } else {
