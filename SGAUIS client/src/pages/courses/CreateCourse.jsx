...
function CreateCourse () {
  ...
  const { coursesCreate, errors: coursesErrors, showSuccessMessage, courseData } = useAuth()
  const onSubmit = handleSubmit(async (values) => {
    await coursesCreate(values)
  })

  return (
    <div className='bg-white text-gray-200 shadow-md rounded-lg px-8 py-6 w-[60%] place-self-center'>
      {showSuccessMessage && (
      ...
      <form onSubmit={onSubmit}>
        ...
        <div className='mb-2'>
          ...
          <input type='text' {...register('descripcion', { required: true })} id='descripcion' className='shadow-sm rounded-md w-full px-3 py-2 border border-gray-300 text-gray-800 focus:outline-none focus:border-sky-600 focus:border-2' placeholder='Ingrese la descripción del curso' />
