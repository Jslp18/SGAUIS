...
import CursosEstudiante from './student/CoursesStudent'
import InfoEstudiante from './student/InfoStudent'
import { useEffect } from "react"
import SGAUIS12 from '../resources/SGA UIS 12.png'
...
function StudentPage() {
  ...
  const { setContentCourse, setSearchContent, setHomeworkCourse, setSearchHomework, setForumCourse, setSearchForum } = useStudent()
    ...
    <div className='text-gray-900 bg-slate-100 body-font shadow w-full overflow-auto'>
      <div className='flex justify-start w-full h-[calc(100vh-9vh)]'>
        ...
        <div className='bg-slate-100 w-5/6 h-[calc(100vh-9vh)] flex justify-center overflow-y-auto px-10 py-6'>
          {showRegisterPage === 'home' && <Student />}
          {showRegisterPage === 'cursos' && <CursosEstudiante />}
          {showRegisterPage === 'info' && <InfoEstudiante />}
        </div>
      </div>
    </div>
  )
...
function Student() {
  ...
  const { viewStudentsCourses } = useStudent()
  ...
  return (
    <img className='place-self-center opacity-85 w-2/5' src={SGAUIS12} alt='SGA UIS: Sistema de Gestión de Aprendizaje Universidad Industrial de Santander.' />
  )
}
