...
import {
    ...
    verTareaEntregada }  from '../api/student'
...
export function StudentProvider({ children }) {
    ...
    const homeworkSubmission = async (idHomework, formData) => {
        try {
            const cookies = Cookies.get()
            const config = {
                headers: {
            ...
            const response = await entregarTarea(idHomework, formData, config)
    ...
    const getHomeworkSubmission = async (idHomework) => {
        try {
          const response = await verTareaEntregada(idHomework)
          return response.data
        } catch (error) {
          console.error(error)
        }
