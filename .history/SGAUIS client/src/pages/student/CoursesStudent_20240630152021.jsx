...
function CoursesStudent() {
  ...
  const { studentCourses, search, errors: commentErrors, showSuccessMessage, getContentCourse, contentCourse, setContentCourse,
    searchContent, setSearchContent, getHomeworksCourse, homeworkCourse, setHomeworkCourse, searchHomework, setSearchHomework,
    getForumsCourse, forumCourse, setForumCourse, searchForum, setSearchForum, commentForumUpload, getCommentsForum, commentForumData,
    setCommentForumData, searchCommentForum, setSearchCommentForum, homeworkSubmissionData, setHomeworkSubmissionData, setSearchHomeworkSubmission,
    selectedFile, setSelectedFile, homeworkSubmission, getHomeworkSubmission } = useStudent()

  const [currentCourse, setCurrentCourse] = useState(null)
  const [currentForum, setCurrentForum] = useState(null)
  const [currentHomework, setCurrentHomework] = useState(null)

  const [homeworkSubmissionYet, setHomeworkSubmissionYet] = useState(null)
  ...
  const agregarEntrega = async (homework) => {
    ...
    const homeworkSubmission = await getHomeworkSubmission(idHomework)
    console.log(homeworkSubmission)
    setHomeworkSubmissionYet(homeworkSubmission)
    reset()
  }
  ...
  const onSubmission = handleSubmit(async (values) => {
    ...
    await homeworkSubmission(currentHomeworkId, formData) // Envía el formData al backend
  })

  // Ver la calificación de la tarea
  const verCalificacion = async (homework) => {
    setSearchHomeworkSubmission(false)
    setHomeworkSubmissionData(null)
    setCurrentHomework(homework)
            ...
            <div className='flex flex-col w-full items-center justify-center'>
              ...
              {showHomeworkPage === 'agregarEntrega' && (
                <div className='bg-white text-gray-200 shadow-gray-400 shadow-2xl flex flex-col py-8 px-8 rounded-lg w-[50%] mt-10'>
                            ...
                            <div className='bg-white sm:p-4 sm:pb-4'>
                              <div className='sm:mt-0 sm:ml-0 sm:text-center'>
                                <h3 className='text-lg leading-6 font-medium text-gray-900' id='modal-title'>
                                  La entrega {homeworkSubmissionData.titulo} ha sido entregada de manera satisfactoria.
                                </h3>
                              </div>
                            </div>
                          </div>
                  ...
                  { homeworkSubmissionYet.estadoEntrega === true ? (
                   
                  ) : (
                    <div>Ya has realizado una entrega a la tarea {currentHomework.nombre} </div>
                  <form onSubmit={onSubmission} encType='multipart/form-data'>
                    <div className='flex flex-col py-6 rounded-lg items-center justify-center'>
                      <div className='mb-4 relative w-full'>
                        <label htmlFor='titulo' className='block text-sm font-medium text-gray-700 mb-2'>Título de la entrega</label>
                        ...
                        {errors.titulo && (<p className='text-rose-400'>Por favor, completa este campo</p>)}
                      </div>
                      <div className='bg-gray-950/10 rounded-lg space-y-4 w-full px-2 py-4'>
                        ...
                        <label htmlFor='pdf' className='block text-sm font-medium text-gray-700 mt-2'> {selectedFile ? (<span>Archivo seleccionado: {selectedFile.name}</span>) : (<span>Cargar contenido PDF</span>)}</label>
                        {errors.pdf && (<p className='text-rose-400'>Por favor, completa este campo.</p>)}</div>
                    </div>
