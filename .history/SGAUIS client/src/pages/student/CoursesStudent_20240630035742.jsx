              ...
              {showForumPage === 'verComentarios' && (
                <div className='flex flex-col w-full'>
                  {search === true && commentForumData.length === 0 && (
                    <div className='flex place-self-center mt-20 w-[80%]'>
                      <h1 className='font-bold text-4xl coinstracking-widest text-[#231F20] text-opacity-35 text-center'>Ups, parece que no hay ninguna respuesta a la discusión asignada en el foro {current}.</h1>
                    </div>
                  )}
                    ...
                    <div className='flex flex-col w-full rounded-lg mt-14'>
                      <div className='rounded-lg overflow-hidden shadow-gray-500 shadow-2xl'>
                        {commentForumData.map((comment, index5) => (
                          <div key={index5} className='relative py-2 px-4'>
                            <p className='text-justify'>{comment.comentario}</p>
                            <span className='absolute bottom-2 right-2 text-sm font-normal'>{comment.usuario.nombre}</span>
                          </div>
                        ))}
                      </div>
                    </div>
