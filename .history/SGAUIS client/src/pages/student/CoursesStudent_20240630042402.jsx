...
function CoursesStudent() {
  ...
  const { studentCourses, search, errors: commentErrors, showSuccessMessage, getContentCourse, contentCourse, setContentCourse, searchContent, setSearchContent,
    ...
    setSearchForum, commentForumUpload, getCommentsForum, commentForumData, searchCommentForum
  } = useStudent()
                            ...
                            <div className='bg-white sm:p-4 sm:pb-4'>
                              <div className='sm:mt-0 sm:ml-0 sm:text-center'>
                                <h3 className='text-lg leading-6 font-medium text-gray-900' id='modal-title'>
                                  El comentario ha sido añadido al foro {currentForum.nombre} de manera satisfactoria.
                                </h3>
                              </div>
                            </div>
                          </div>
              ...
              {showForumPage === 'verComentarios' && (
                <div className='flex flex-col w-full items-center justify-center'>
                  ...
                  {commentForumData.length !== 0 && showForumPage === 'verComentarios' && (
                    <div className='flex flex-col w-[80%] rounded-lg mt-14'>
                      <div className='flex flex-row items-center justify-center gap-3'>
                      ...
                      <div className='space-y-5 mt-4'>
                        {commentForumData.map((comment, index5) => (
                          <div key={index5} className='relative p-6 rounded-lg shadow-gray-500 shadow-lg bg-white/80'>
                            <p className='text-justify'>{comment.comentario}</p>
                            <span className='absolute bottom-5 right-4 text-sm font-medium block bg-gradient-to-tr from-neutral-900 to-gray-700 bg-clip-text font-sans leading-relaxed text-transparent antialiased'>{comment.usuario.nombre} - {comment.usuario.codigo}</span>
                            <span className='absolute bottom-1 right-4 text-sm font-normal block bg-gradient-to-tr from-sky-900 to-sky-700 bg-clip-text font-sans leading-relaxed text-transparent antialiased'>{comment.usuario.correo}</span>
                          </div>
                        ))}
                      </div>
                    </div>
