                ...
                <div className='flex flex-col w-full items-center justify-center'>
                  ...
                  {commentForumData.length !== 0 && showForumPage === 'verComentarios' && (
                    <div className='flex flex-col w-[70%] rounded-lg mt-14'>
                      ...
                      <div className='overflow-hidden shadow-gray-500 space-y-5'>
                        {commentForumData.map((comment, index5) => (
                          <div key={index5} className='relative py-4 px-4 rounded-lg' style={{ backgroundColor: getRandomColor() }}>
                            <p className='text-justify'>{comment.comentario}</p>
                            <span className='absolute bottom-4 right-4 text-sm font-normal p-2'>{comment.usuario.nombre} - {comment.usuario.codigo}</span>
                            <span className='absolute bottom-0 right-4 text-sm font-normal  p-2'>{comment.usuario.correo}</span>
                          </div>
                        ))}
                      </div>
                    </div>
