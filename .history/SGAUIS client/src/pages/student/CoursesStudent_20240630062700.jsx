                    ...
                    <tbody>
                      {currentHomeworks.map((homework, index2) => (
                        <tr key={index2} className='bg-zinc-200 border border-transparent hover:bg-zinc-300'>
                          ...
                          <td className='px-6 py-3'>
                            {isPastDueDate(homework.fechaEntrega) ? (
                              <p>No es posible realizar esta entrega</p>
                            ) : (
                              <div className='flex flex-row gap-3'>
                                <button onClick={() => { entregarTarea(forum) }} className='flex flex-row w-[40%] items-center justify-center px-2 group cursor-pointer text-md bg-slate-400 hover:bg-slate-500 focus:bg-slate-600 focus:text-white rounded-2xl border-2 border-[#2D2D2D] gap-3'><svg xmlns='http://www.w3.org/2000/svg' fill='#FFF' viewBox='0 0 24 24' strokeWidth='1.5' stroke='currentColor' className='text-[#2D2D2D] w-6 h-6 group-focus:text-[#2D2D2D] group-focus:fill-white'><path strokeLinecap='round' strokeLinejoin='round' d='m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125' /></svg><p className='text-black font-normal group-focus:text-black'>Agregar entrega</p></button>
                                <button onClick={() => { verCalificación(forum) }} className='flex flex-row w-[40%] items-center justify-center px-2 group cursor-pointer text-md bg-zinc-400 hover:bg-zinc-500 focus:bg-zinc-600 focus:text-white rounded-2xl border-2 border-[#2D2D2D] gap-3'><svg xmlns="http://www.w3.org/2000/svg" fill="#FFF" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className='text-[#2D2D2D] w-6 h-6 group-focus:text-[#2D2D2D] group-focus:fill-white'><path strokeLinecap="round" strokeLinejoin="round" d="M7.5 8.25h9m-9 3H12m-9.75 1.51c0 1.6 1.123 2.994 2.707 3.227 1.129.166 2.27.293 3.423.379.35.026.67.21.865.501L12 21l2.755-4.133a1.14 1.14 0 0 1 .865-.501 48.172 48.172 0 0 0 3.423-.379c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0 0 12 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018Z" /></svg><p className='text-black font-normal group-focus:text-black'>Ver calificación</p></button>
                              </div>
                            )}
                          </td>
                        </tr>
