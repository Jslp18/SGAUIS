            ...
            <div className='flex flex-col items-center justify-center w-full'>
              ...
              {showForumPage === 'verComentarios' && (
                <div className='flex flex-col w-full items-center justify-center'>
                  {search === false && commentForumData.length === 0 && (
                    <div className='flex place-self-center mt-20 w-[80%]'>
                  ...
                  {search === true && commentForumData.length === 0 && (
                    <div className='flex place-self-center mt-20 w-[80%]'>
                      <h1 className='font-bold text-4xl coinstracking-widest text-[#231F20] text-opacity-35 text-center'>Ups, parece que no hay ninguna respuesta a la discusión asignada en el foro {currentForum.titulo}.</h1>
                    </div>
                  )}
                  {commentForumData.length !== 0 && (
                    <div className='flex flex-col w-[70%] rounded-lg mt-14'>
                      <div className='flex flex-row items-center justify-center gap-3'>
                        <h1 className='font-bold text-xl coinstracking-widest text-neutral-700 text-center'>Respuestas del foro</h1>
                        <h1 className='font-bold text-xl coinstracking-widest text-amber-700/60 text-center'>{currentForum.titulo}</h1>
                      </div>
                      <div className='rounded-lg overflow-hidden shadow-gray-500 space-y-5'>
                        {commentForumData.map((comment, index5) => (
                          <div key={index5} className='relative py-4 px-4 bg-[randomizar aquí el color '>
                            <p className='text-justify'>{comment.comentario}</p>
                            <span className='absolute bottom-4 right-4 text-sm font-medium'>{comment.usuario.nombre} - {comment.usuario.codigo}</span>
                            <span className='absolute bottom-0 right-4 text-sm font-medium  '>{comment.usuario.correo}</span>
                          </div>
                        ))}
                      </div>
                    </div>
