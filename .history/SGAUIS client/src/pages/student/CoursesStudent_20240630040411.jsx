                    ...
                    <div className='flex flex-col w-[70%] rounded-lg mt-14'>
                      ...
                      <div className='rounded-lg overflow-hidden shadow-gray-500 space-y-5'>
                        {commentForumData.map((comment, index5) => (
                          <div key={index5} className='relative py-2 px-4 bg-[randomizar aquí el color '>
                            <p className='text-justify'>{comment.comentario}</p>
                            <span className='absolute bottom-2 right-2 text-sm font-normal'>{comment.usuario.nombre} - {comment.usuario.codigo}</span>
                            <span className='absolute bottom-2 right-2 text-sm font-normal'>{comment.usuario.nombre} - {comment.usuario.correo}</span>
                          </div>
                        ))}
                      </div>
                    </div>
